...
const Show = ({ semesters, limit }) => {
    ...
    const confirmDelete = () => {
        router.delete(`hoc-ky/${idToDelete}`);
        handleClosePopup();
    };
        ...
        <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
            ...
            <div className='flex flex-col justify-between md:flex-row gap-5 mb-4'>
                ...
                <form onSubmit={handleSearchSubmit} className='flex items-center gap-5'>
                    ...
                    {/* Add */}
                    <Link href="/admin/hoc-ky/them" className="bg-graydark hover:opacity-80 text-white font-bold py-2 px-4 rounded text-center">
                        Thêm
                    </Link>
                </form>
                ...
                <table className="w-full table-auto">
                    <thead>
                        <tr className="bg-gray-2 text-left dark:bg-meta-4">
                            ...
                            <th className="min-w-[150px] py-4 px-4 font-medium text-black dark:text-white">Kỳ</th>
                            <th className="min-w-[150px] py-4 px-4 font-medium text-black dark:text-white">Bắt đầu</th>
                            <th className="min-w-[150px] py-4 px-4 font-medium text-black dark:text-white">Kết thúc</th>
                            ...
                            semesters.data.map((semester, index) => (
                                <tr key={semester.id}>
                                    ...
                                    <td className="border-b border-[#eee] py-4 px-4 dark:border-strokedark">
                                        <p className="text-black dark:text-white">{semester.block}</p>
                                    </td>
                                    <td className="border-b border-[#eee] py-4 px-4 dark:border-strokedark">
                                    ...
                                    <td className="border-b border-[#eee] py-4 px-4 dark:border-strokedark">
                                        <p className="text-black dark:text-white">{semester.year}</p>
                                    </td>
                                    <td className="border-b border-[#eee] py-4 px-4 dark:border-strokedark">
                                        <div className="flex items-center space-x-3.5">
                                            <Link href={`/admin/hoc-ky/${semester.id}/sua`} className="hover:text-primary">
                                                <i
                                                    className="fa-regular fa-pen-to-square text-xl"
                                                    title="Chỉnh sửa"
                                                ></i>
                                            </Link>
                                            <button className="hover:text-primary" onClick={() => handleDeleteClick(semester.id)}>
                                                <i
            ...
            {/* Pagination */}
            <div className='my-6'>
                <Pagination link={semesters.links} onPageChange={handlePageChange} />
            </div>
